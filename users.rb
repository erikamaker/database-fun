...
# Create an index on the "username" column if it doesn't exist
db.execute('CREATE INDEX IF NOT EXISTS index_username ON users (username)')
...
def retrieve_user_count(db)
  count = db.execute('SELECT COUNT(*) FROM users').first[0]
  puts "Total users: #{count}"
end

# Retrieve the total number of users
retrieve_user_count(db)
